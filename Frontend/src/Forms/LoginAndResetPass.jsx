...
import { Box, InputLabel, TextField, IconButton, InputAdornment } from "@mui/material";
...
function LoginAndResetPass() {
    ...
    const handleClickShowConfirmPassword = () => setShowConfirmPassword(!showConfirmPassword); // تبديل حالة الرؤية لتأكيد كلمة المرور
            ...
            <div className="flex-col justify-center items-center text-center">
                {routeWords == "auth" ?
                    <>
                        ...
                        <div className="subTitle font-thin text-sm">
                            Don’t have an account? <Link className="text-[#6C63FF]">Sign Up</Link>
                        </div>
                    </> :
                    <>
                        {
                ...
                <div className="route-info">
                    <form onSubmit={handleSubmit(onSubmit)} className="text-start">
                        <Box>
                            <InputLabel className="mt-5"><div className="text-black">{routeWords == "send-email"?"Email address":"Email"}</div></InputLabel>
                            {
                        ...
                        {routeWords != "send-email"?<Box>
                            <InputLabel className="mt-5"><div className="text-black">Password</div></InputLabel>
                            <TextField 
                                ...
                                placeholder={routeWords != "send-email" && routeWords != "auth"?"enter new password":"enter password"}
                                InputProps={{
                                    endAdornment: (
                            ...
                            {routeWords != "send-email" && routeWords != "auth"?<>
                                ...
                                <div className="text-gray-400">Must be at least 8 characters</div>
                            </div>
                            <div  className="flex">
                                <DoneIcon color="disabled" />
                                <div className="text-gray-400">Does not contain your email address</div>
                            </div>  
                            </>:<><div><Link to={"/send-email"}>Forgot Password?</Link></div></>}            
                        </Box>:""}
                        
                        {routeWords != "auth" && routeWords != "send-email" ? (
                            <Box>
                                <InputLabel className="mt-5"><div className="text-black">Confirm Password</div></InputLabel>
                                <TextField
                                    className="bg-white rounded-md"
