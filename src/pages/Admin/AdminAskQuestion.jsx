...
function AdminAskQuestion() {
  ...
  const handleSubmitAnswer = async (id) => {
    ...
    setCurrentAnswer("");
    // Refetch questions after sending answer
    const data = await fetchAskQuestion({ token });
    setQuestions(data.data);
  };
          ...
          <Container maxWidth="lg" sx={{ py: 4 }}>
            <Grid container spacing={3}>
              <Grid item xs={12} md={8}>
                {questions.length == 0 ? (
                  <div className="text-center text-2xl text-[#6C63FF]">
                  ...
                  questions.map((row, index) => (
                    ...
                    >
                      ...
                      <Typography
                        variant="h6"
                        sx={{ mb: 2, fontWeight: 500, fontSize: "1.1rem" }}
                      >
                        {row.question}
                      </Typography>
                  ...
                  <Box sx={{ mt: 3 }}>
                    {!searchQuery.trim() ? (
                      <div className="text-center">
                        You can search for any question.
                      </div>
                    ) : recentQuestions.length == 0 ? (
                      `There is no question about "${searchQuery}"`
                    ) : (
                      <>
